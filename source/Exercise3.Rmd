...
knitr::opts_knit$set(root.dir = "C:/Users/jesto/sfuvault/SISMID2021/Flu_data")
```
...
Phylogenetic trees help us to understand the genetic distance between samples, as well as the overall structure of a set of samples. However, they are still involve simplification of the rich information contained within sequences and therefore some information will, in practice, always be lost in translation. There are also many different approaches for building trees, and in all but the simplest case you will get a different answer depending on which method you use. 
...
Now that the tree is rooted we can think of the left-hand side as 'ancestral' and the right side as more 'recent'. 
...
Rooted trees, such as the one we just made, can be used to estimate rates of evolution. Unlike a regular clock which measures the passing of seconds, minutes or hours, a 'molecular clock' measures the accumulation of genetic mutations over time. To estimate the clock rate of our rooted tree, we can perform a regression over all tips of the number of mutations from the root against the time since the root.
...
The coefficient of `yearFromRoot` is ~7.7. This is our estimate of the number of substitutions per year for the HA segment of the H3N2 influenza genomes. What is the substitution rate per site per year? Therefore, how many days would you expect it to take to accumulate 1 mutation in a transmission chain? Lastly, given your answer to the previous question and the fact that the generation time of influenza is 2-3 days, do you think that reconstructing influenza transmission trees from HA sequences is a good approach?

## Introduction: maximum parsimony phylogenies

An alternative to distance-based phylogenies is maximum parsimony phylogenies.

In general terms, the scientific principle of parsimony can be described as the idea that the simplest explanation that can explain the data is  preferred. Maximum parsimony phylogenetic reconstruction therefore seeks trees which minimize the number of evolutionary changes (substitutions) between ancestor and descendant. This makes for a simple and easy-to-motivate methodology, which is most appropriate for sequences with low divergence i.e. most sequences are very similar to one another.  

In an ideal world we would search over the space of all possible trees to find the one which is most parsimonious. However, in reality this is usually computationally impossible unless you have very few tips in your tree. Many algorithms have been designed to instead search broadly over this space in a clever way - to try and find a decently parsimonious tree without needing to check too many. These algorithms generally take a strategy of (1) pick an initial tree and calculate it's parsimony, (2) propose some small changes to the tree and calculate the new tree's parsimony, (3) keep the new tree if it leads to a better parsimony, and (4) repeat until the parsimony score stops improving. 


Parsimony-based phylogenetic reconstruction is implemented in the package
phangorn. It requires a tree (in ape's format, i.e. a phylo object) and the
original DNA sequences in phangorn's own format, phyDat. We convert the
data and generate a tree to initialize the method:

10. Create a maximum parsimony tree for the US flu data using the package ``phangorn``. This requires an initial tree (you can use ``ape`` to make this, as in the previous section) as well as the original DNA sequences, which must be converted to ``phangorn``'s ``phyDat`` format, as follows:
...
Our final phylogenetic tree building approach is maximum likelihood. The idea here is very similar to maximum parsimony, in that we try to search over the space of all possible trees in a smart way to find the 'best' one, but this time we want to maximize some pre-defined likelihood function rather than maximizing parsimony.

A big benefit of this approach is that the likelihood to be maximized can be defined by any model of sequence evolution we choose, which makes it much more flexible. As well as optimizing over the space of trees, we can also simultaneously optimize over the parameters of our chosen model - to find the best tree and the best model parameters at the same time. A drawback, however, is that this means we need to decide what is the most appropriate to use!

A common choice is the GTR + $\Gamma$(4) model (GTR = global reversible time). In this model, all possible substitutions (all transitions, transversions) are allowed to have different rates as well as the substitution rate being allowed to vary across sites. 
...
14. Find a maximum likelihood phylogeny for the US flu data, using ``optim.pml``. Remember to find a new initial tree, using your new sequences with missing data removed. 

```{r}
#Make the initial tree
tree_init <- nj(dist.dna(seqs3, model = "TN93"))
#Find it's (log) likelihood
...
The results object (here named ``fit``) contains not just the optimal tree (``fit$tree``), but also other useful info - explore these.  
...
14. Lastly, plot your maximum likelihood phylogeny for the US flu data (just like for the other methods).
...
Overall, we find that for this dataset all methods return fairly similar trees. This dataset is relatively simple, with not too many samples and low diversity, so this is not unexpected. For larger and more complex datasets, you will likely find much more stark differences between different methods. It's usually a good idea to test out a few different tree building approaches before drawing any conclusions about your data. 
...
- **Maximum parsimony.** *Pros:* intuitive, works well for datasets with small amounts of genetic variation. *Cons:* simplistic evolutionary model, computationally intensive, works poorly for datasets with large amounts of genetic variation or where mutation rates vary across the tree. 
...
Remember that we don't expect you to finish the whole tutorial for all 3 datasets within one 50 minute session. But hopefully you can make a good start, ask lots of questions, and then continue in the open work sessions or outside of our schedule if you want to. 


## Extension tasks and additional reading

A. Here are several alternatives to ``nj`` for building phylogenetic trees:
...
B. Development of methods for comparison of phylogenetic trees is an active area of research. Investigate the R packages ape and treespace, or look up alternatives, for ways to compare trees, and apply these to your TB/COVID trees. Here is a document about treespace that might be useful  https://cran.r-project.org/web/packages/treespace/vignettes/introduction.html

C. If you are already familiar with phylogenetic inference, but haven’t yet used or made timed trees, you can explore ‘treedater’ (an R package by Erik Volz https://github.com/emvolz/treedater) or the python package TreeTime https://github.com/neherlab/treetime by Richard Neher. 
