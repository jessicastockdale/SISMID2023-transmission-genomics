...
knitr::opts_knit$set(root.dir = "C:/Users/jesto/sfuvault/SISMID2021/TB_data")
```

# Reconstructing transmission pathways

In this exercise we use TransPhylo to reconstruct transmission trees. Before starting this exercise make sure you have watched the video for ‘TransPhylo and the math behind it’ in which we introduce the package.
...
First, we are going to use TransPhylo's outbreak simulator to simulate a small outbreak. In this simulation we will know exactly who-infected-who. When we then use TransPhylo to reconstruct the transmission pathway (pretending that we don't already know it), we can compare the results to see how well we did. 
...
- `off.r`: the basic reproduction number (which forms the mean of the negative binomial offspring distribution)

- `w.shape` and `w.scale`: the shape and scale of the gamma distributed generation time. (Will also be used for the sampling time distribution unless we define `ws.shape` and `ws.scale`)
...
w.shape=10
w.scale=0.1 # Gives a mean of 1 year
...
3. Simulate and plot an outbreak with your chosen parameters which begins in 2005 and stops being observed in 2008. 
...
In this section we are going to pretend that we don't actually know who-infected-who in our simulated outbreak, and instead use TransPhylo to infer this. We'll start with our phylogeny `p`, which you'll recall we removed all transmission information from. 
...
Unlike Outbreaker which inferred transmission directly from genomes and sampling dates, TransPhylo requires an input phylogenetic tree. So usually we will use our favourite phylogenetic tree building method as in exercise 2 to get an phylogeny, and then we are ready to run TransPhylo. A recent improvement to TransPhylo means that you can now input a whole set of potential phylogenies rather than having to pick just one - this lets you build uncertainty in your tree into the analysis. We won't be covering that in this tutorial, but feel free to ask us about it if you are interested. 
...
Although this seems a little circular for this tutorial because you already had the object `ptree` earlier, remember that in a regular (unsimulated) analysis you would begin this process here at step 8. 

TransPhylo needs 4 things to run the transmission inference: the phylogenetic tree we already have, that leaves the assumed generation time distribution, the date at which observation of the outbreak stopped and the number of iterations to run the MCMC algorithm for. 

 In the simulation we used generation time parameters (10, 0.1), so we can give TransPhylo the best chance of success by inputting those correct values here. In a real-data scenario you won't know the true generation time of course, so you should use something sensible for the pathogen you are working with (perhaps by looking up estimates people have obtained for that pathogen using contact tracing or other data). Similarly for the time outbreak observation ended, in our simulated outbreak we used `dateT=2008` so we can input that here. For real data, you should use whatever date you stopped observing your outbreak of interest. If you're doing a real-time genomic epidemiology investigation, that date might be today. If you're absolutely confident that your outbreak was over with the last sampled case in your dataset, you can set this date to `Inf` (infinity) - this is useful when you are analysing old outbreaks in which you know there was no more transmission. Inputting a `dateT` allows TransPhylo to account for the fact that individuals who became infected just before T have a lower probability of being sampled.
...
Now we are ready to run out transmission inference. 
...
Again like Outbreaker, we need these trace plots to look like stable fuzzy caterpillars in order for our results to be reliable. We see that for the input parameters we chose, they are not. This implies that we should rerun ``inferTTree`` with a highr number of `mcmcIterations`. 
...
In a real outbreak we wouldn't have the true tree to compare to of course, but we could at this point dive into the details of who-infected-who and see if any of our transmission pairs are especially meaningful - for example did any live, work or socialize together? With sufficient metadata, we could also investigate if the hosts inferred to have infected the most people had anything in common - were they young, old, very social, in high-contact jobs? In addition to looking at the transmission tree, we are also able to make a variety of plots exploring other aspects of the analysis...
...
The diagonal of these matrices will always be 0, since hosts cannot infect themselves. Outside of that, we see that many pairs are inferred to have between 3 and 5 unsampled intermediates. This somewhat matches the value of sampling rate parameter `pi = 0.25` that we used in our simulation. Note that this figure must be interpreted carefully though, because all pairs in this matrix are not equally probable. A less probable transmission pair will be more likely to have a higher number of unsampled intermediates between them when they do occur, but nature of being more distant.
...
We infer that most unsampled cases occurred back towards the root of the tree. The generation and sampling distributions are fairly smooth and unimodal. How well do these match the distributions you simulated from? 

18. Lastly, make 2 plots showing the distribution of inferred infection time for individuals '1' and '2' and the offspring distribution for individuals '1' and '2'.
...
You could also make these plots for any of the other hosts. 
...
* TB: gen. time ~ gamma(shape = 1.3, rate = 0.3) years, and samp. time ~ (shape = 1.1, rate = 0.4) years (since our time scale is years). 
...
We could also compare these trees with various numeric measures. The underlying phylogenetic tree topology is the same, so tools such as `treespace` that was mentioned in exercise 3 won't help here. The main differences in our trees comes from the location of the red stars, who-infected-who, and the number/location of unsampled individuals. 
...
C. Try visualising your inferred transmission tree(s) using [VisNetwork](https://datastorm-open.github.io/visNetwork/) - a network visualisation package in R. Some sample code is available in transphylo_extras.R which is available from the module webpage. This also contains various extra functions for visualising the output of TransPhylo  - try these out on any of your inferred transmission trees. 

D. If you're interested to learn about simultaneous inference of multiple transmission trees from a set of phylogenetic trees in TransPhylo, you can try out the [this tutorial](https://xavierdidelot.github.io/TransPhylo/articles/multitree.html). 
